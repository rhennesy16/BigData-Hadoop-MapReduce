...
public class NoStops extends Configured implements Tool {
	...
	public static class RoutesMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		private static final int HAS_STOPS = 7;
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if(STOPS == Integer.parseInt(itr[HAS_STOPS]) && StringUtils.isNumeric(itr[AIRLINE_ID])) {
	...
	public static class AirlineRoutesReducer	extends Reducer<Text, Text, Text, Text> {
		private Text airlineName = new Text();
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "NonStop");
		job.setJarByClass(NoStops.class);
		...
		job.setReducerClass(AirlineRoutesReducer.class);
	...
	public static void main(String[] args) throws Exception {
		int ecode = ToolRunner.run(new NoStops(), args);
		System.exit(ecode);
	}
}
